...
AUTO_PRINT_STAGE_FIRST = os.environ.get("AUTO_PRINT_STAGE_FIRST", "1") == "1"
...
def write_nginx_vhosts(domains: list, proxy_pass: str) -> None:
    ...
    def cert_paths(cert_name: str) -> tuple[str, str]:
        fullchain = os.path.join(LE_LIVE_DIR, cert_name, "fullchain.pem")
        privkey = os.path.join(LE_LIVE_DIR, cert_name, "privkey.pem")
        return fullchain, privkey

    def cert_exists(cert_name: str) -> bool:
        fullchain, privkey = cert_paths(cert_name)
        return os.path.isfile(fullchain) and os.path.isfile(privkey)

    def select_cert_name(apex: str) -> Optional[str]:
        if cert_exists(apex):
            return apex
    ...
    cert_groups = []  # list of (apex, names, chosen_cert_name)
    for apex, names in groups.items():
        if names:
            chosen = select_cert_name(apex)
            if chosen is not None:
                cert_groups.append((apex, names, chosen))
    ...
    if cert_groups:
        for apex, names, chosen_cert_name in cert_groups:
            server_names = " ".join(names)
            fullchain, privkey = cert_paths(chosen_cert_name)
...
def main() -> None:
    print("front-web controller starting... (staging/prod cert selection enabled; reload nginx to apply)", flush=True)
    ...
    while True:
        ...
        if new_domain_mtime != last_domain_mtime or new_proxy_mtime != last_proxy_mtime:
            try:
                ...
                print("Config changed; reloaded (staging/prod cert selection enabled):")
